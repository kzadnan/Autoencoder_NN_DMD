# -*- coding: utf-8 -*-
"""
Created on Tue Apr 15 21:28:02 2025
...
def write_lammps_trajectory(recon_data, matrix, num_atoms, sorted_timesteps, box_bounds, valid_indices, mean, std, output_file="recon_nvt.tj"):
    ...
    with open(output_file, 'w') as f:
        for t, timestep in enumerate(sorted_timesteps):
            ...
            for i in range(num_atoms):
                # Get id and type from original matrix
...
# Reconstruct data
recon_data = np.zeros((X.shape[0], X.shape[1]))  # Shape: (T, Dom 3 * num_atoms)
with torch.no_grad():
